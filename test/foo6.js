...
var json_data_1 = require("./foo6.json");

contract('ContentReward TEST foo6', function() {
  ...
  it("0. basic part", async() => {
    ...
    await cr.sendTransaction({from: web3.eth.accounts[6], to: cr.address, value: 15*10**18});
    // pubslish content
  ...
  it("10. someone publish more than one comment to the same content", async() => {
    const cr = await ContentReward.deployed();
    for (let foo_content_id in content_act_1) {
      foo_ct = content_act_1[foo_content_id];
      foo_comments = foo_ct['comments'];
      for (let foo_comment_id in foo_comments) {
        if (foo_comments[foo_comment_id]['delete']) {
          ...
          try {
            await cr.commentContent(foo_content_id, foo_content_creator, user_address[foo_content_creator], "401000", timestamp, user_reputation_1[foo_content_creator], false, "");
          } catch (err) {}
        }
      }
      // calculate value
