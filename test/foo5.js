...
var json_data_1 = require("./foo5.json");

contract('ContentReward TEST foo5 ', function() {
  ...
  it("0. basic part", async() => {
    ...
    await cr.sendTransaction({from: web3.eth.accounts[5], to: cr.address, value: 15*10**18});
    // pubslish content
  ...
  it("10. publish favor to one's own content", async() => {
    const cr = await ContentReward.deployed();
    // publish comment by content owner
    for (let foo_content_id in content_act_1) {
      foo_ct = content_act_1[foo_content_id]
      foo_likes = foo_ct['likes'];
      for (let foo_like of foo_likes) {
