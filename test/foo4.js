...
var json_data_1 = require("./foo4.json");

contract('ContentReward TEST foo4', function() {
  ...
  it("0. basic part", async() => {
    ...
    await cr.sendTransaction({from: web3.eth.accounts[4], to: cr.address, value: 15*10**18});
    // pubslish content
  ...
  it("10. publish comment to one's own content", async() => {
    const cr = await ContentReward.deployed();
    for (let foo_content_id in content_act_1) {
      foo_ct = content_act_1[foo_content_id];
      foo_comments = foo_ct['comments'];
