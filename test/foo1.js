...
var json_data_1 = require("./foo1.json");

contract('ContentReward TEST foo1', function() {
  ...
  it("0. basic part", async() => {
    ...
    await cr.sendTransaction({from: web3.eth.accounts[1], to: cr.address, value: 15*10**18});
    // pubslish content
  ...
  it("10. delete content", async() => {
    const cr = await ContentReward.deployed();

    for (let foo_content_id in content_act_1) {
      if (content_act_1[foo_content_id]['delete']) {
        await cr.deleteContent(foo_content_id);
      }
    }
