...
var json_data_1 = require("./foo8.json");

contract('ContentReward TEST foo8', function() {
  ...
  it("0. basic part", async() => {
    ...
    await cr.sendTransaction({from: web3.eth.accounts[8], to: cr.address, value: 15*10**18});
    // pubslish content
  ...
  it("5. publish content", async() => {
    ...
    for (let foo_content_id in content_act_1) {
      ...
      // calculate value
      const foo_content = await cr.contentsDetail.call(foo_content_id);
      // foo_content_value = result_1[foo_content_id]['before']['content'];
      // comment value
      foo_comment_value = result_1[foo_content_id]['before']['comment'];
      // console.log(foo_content_id + ": " + foo_content[1]);
      assert.equal(foo_content[1], foo_comment_value, "equal");
      // like value
      foo_like_value = result_1[foo_content_id]['before']['like'];
      // console.log(foo_content_id + ": " + foo_content[0]);
      assert.equal(foo_content[0], foo_like_value, "equal");
      // share value
      foo_share_value = result_1[foo_content_id]['before']['share'];
      // console.log(foo_content_id + ": " + foo_content[2]);
      assert.equal(foo_content[2], foo_share_value, "equal");
    }
  });

  it("10. delete comment until 0", async() => {
    const cr = await ContentReward.deployed();
    // delete comment until 0
    for (let foo_content_id in content_act_1) {
      foo_ct = content_act_1[foo_content_id]
      foo_comments = foo_ct['comments'];
      for (let foo_comment_id in foo_comments) {
        if (foo_comments[foo_comment_id]['delete']) {
          foo_comment_creator = foo_comments[foo_comment_id]['comment_creator']
          foo_reputation = user_reputation_1[foo_comment_creator];
          await cr.deleteComment(foo_content_id, foo_comment_creator, foo_comment_id, timestamp, foo_reputation);
        }
      }
      // calculate value
      const foo_content = await cr.contentsDetail.call(foo_content_id);
      // foo_content_value = result_1[foo_content_id]['after']['content'];
      // comment value
      foo_comment_value = result_1[foo_content_id]['after']['comment'];
      // console.log(foo_content_id + ": " + foo_content[1]);
      assert.equal(foo_content[1], foo_comment_value, "equal");
      // like value
      foo_like_value = result_1[foo_content_id]['after']['like'];
      // console.log(foo_content_id + ": " + foo_content[0]);
      assert.equal(foo_content[0], foo_like_value, "equal");
      // share value
      foo_share_value = result_1[foo_content_id]['after']['share'];
      // console.log(foo_content_id + ": " + foo_content[2]);
      assert.equal(foo_content[2], foo_share_value, "equal");
    }
    ...
    for (let foo_user_id in user_address) {
      ...
      ttc_expected = result_1['ttc']['after'];
