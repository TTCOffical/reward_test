...
var json_data_1 = require("./foo9.json");

contract('ContentReward TEST foo9', function() {
  ...
  it("0. basic part", async() => {
    ...
    await cr.sendTransaction({from: web3.eth.accounts[9], to: cr.address, value: 15*10**18});
    // pubslish content
  ...
  it("10. delete favor until 0", async() => {
    const cr = await ContentReward.deployed();
    // delete favor until 0
    for (let foo_content_id in content_act_1) {
      foo_ct = content_act_1[foo_content_id]
      foo_likes = foo_ct['likes'];
      for (let foo_like of foo_likes) {
        if ( foo_like['delete'] ) {
          foo_like_creator = foo_like['like_creator'];
          foo_reputation = user_reputation_1[foo_like_creator];
          await cr.cancelLikeContent(foo_content_id, foo_like_creator, timestamp, foo_reputation);
        }
      }
      // calculate value
      const foo_content = await cr.contentsDetail.call(foo_content_id);
